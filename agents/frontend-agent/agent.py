...
class ReadMemoryTool(BaseTool):
    name = "read_memory"
    description = "Read architecture or backend manifest from memory."
...
class WriteMemoryTool(BaseTool):
    name = "write_memory"
    description = "Store frontend manifest to memory."
...
class FrontendAgent(BaseAgent):
    ...
    @property
    def max_input_tokens(self): return 10000
    @property
    def max_output_tokens(self): return 5000
    ...
    async def _build_task_prompt(self, ctx: AgentContext) -> str:
        ...
        api_url = ctx.payload.get("api_url", "http://localhost:8000")
        name = ctx.payload.get("project_name", "generated-app")
        return TASK_PROMPT.format(architecture=arch, api_url=api_url, project_name=name)
    ...
    async def _reflect(self, ctx: AgentContext, result: dict) -> None:
        if "files" in result:
            async with httpx.AsyncClient(timeout=10) as c:
                await c.post(f"{MEMORY_URL}/memory", json={"content": f"Frontend manifest: {json.dumps(result)}", "agent_id": AGENT_ID, "memory_type": "agent_memory", "ttl_days": 30})
        try:
...
def create_agent():
    return FrontendAgent(AGENT_ID, AnthropicProvider(model="claude-sonnet-4-6"))
