...
from file_tools import ReadFileTool, RunCommandTool
...
class ReadMemoryTool(BaseTool):
    name = "read_memory"
    description = "Read backend manifest from memory."
...
class WriteMemoryTool(BaseTool):
    name = "write_memory"
    description = "Store QA report to memory."
...
class QAAgent(BaseAgent):
    ...
    @property
    def max_output_tokens(self): return 2000
    ...
    async def _build_task_prompt(self, ctx: AgentContext) -> str:
        manifest = json.dumps(ctx.payload.get("backend_manifest", ctx.payload))
        name = ctx.payload.get("project_name", "generated-app")
        return TASK_PROMPT.format(backend_manifest=manifest, project_name=name)
    ...
    async def _parse_result(self, ctx: AgentContext) -> dict:
        ...
        return {"deploy_approved": False, "error": "no_output"}

    async def _reflect(self, ctx: AgentContext, result: dict) -> None:
        async with httpx.AsyncClient(timeout=10) as c:
            await c.post(f"{MEMORY_URL}/memory", json={"content": f"QA report: {json.dumps(result)}", "agent_id": AGENT_ID, "memory_type": "agent_memory", "ttl_days": 30})
        try:
            await publish(task_completed(ctx.task_id, {"deploy_approved": result.get("deploy_approved", False)}, AGENT_ID), stream="task:completed")
        except Exception: pass


def create_agent():
    return QAAgent(AGENT_ID, AnthropicProvider(model="claude-haiku-4-5-20251001"))
