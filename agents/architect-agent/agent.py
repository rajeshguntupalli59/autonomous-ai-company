...
class ReadMemoryTool(BaseTool):
    name = "read_memory"
    description = "Read PRD or context from memory."
...
class WriteMemoryTool(BaseTool):
    name = "write_memory"
    description = "Store architecture to memory."
...
class ArchitectAgent(BaseAgent):
    ...
    @property
    def max_input_tokens(self): return 8000
    @property
    def max_output_tokens(self): return 4000
    ...
    async def _build_task_prompt(self, ctx: AgentContext) -> str:
        prd = json.dumps(ctx.payload.get("prd", ctx.payload))
        return TASK_PROMPT.format(prd=prd)

    async def _inject_memory(self, ctx: AgentContext) -> str:
        async with httpx.AsyncClient(timeout=5) as c:
            try:
                r = await c.get(f"{MEMORY_URL}/memory/search", params={"query": "PRD product requirements", "agent_id": AGENT_ID, "top_k": 2, "inject": True})
    ...
    async def _reflect(self, ctx: AgentContext, result: dict) -> None:
        if "stack" in result:
            async with httpx.AsyncClient(timeout=10) as c:
                await c.post(f"{MEMORY_URL}/memory", json={"content": f"Architecture: {json.dumps(result)}", "agent_id": AGENT_ID, "memory_type": "agent_memory", "ttl_days": 30})
        try:
            await publish(task_completed(ctx.task_id, {"routes": len(result.get("api_routes", []))}, AGENT_ID), stream="task:completed")
        except Exception: pass


def create_agent():
    return ArchitectAgent(AGENT_ID, AnthropicProvider(model="claude-sonnet-4-6"))
